import type { Metadata } from "next";
import { StorePage, Section } from "@/components/StorePage";

export const metadata: Metadata = {
  title: "サポート — 漢字カタチ入力",
  description: "漢字カタチ入力の使い方・よくある質問・お問い合わせ先。",
};

export default function Support() {
  return (
    <StorePage title="サポート" updated="2026-08-01">
      <Section title="使い方">
        <p>
          「かたち（位置関係）」を選び、続けて「部品」を選ぶと候補が出ます。
          たとえば <b>左右</b> → <b>日</b> → <b>月</b> で <b>明</b>。
          読みが分からない漢字を、形だけで引けます。
        </p>
        <ul>
          <li><b>かたち</b>の行 … 左右・上下・全かこみ など17種（常に出ています）</li>
          <li><b>部首・偏旁</b>タブ … かなキーボードでは打てない部品を画数別に</li>
          <li>
            <b>読みでさがす</b>タブ … 部品の読みから引く。タップでかたちコードに足し、
            長押しでその字をそのまま入力（システムキーボードでは「読み」ボタンで
            12キーフリックのかな面が出ます）
          </li>
          <li>
            <b>履歴・★・着せ替え</b> … 打つ場所と混ざらないよう上部にまとめてあります。
            候補や履歴の字を長押しするとお気に入りに入ります
          </li>
        </ul>
      </Section>

      <Section title="システムキーボードでの流れ">
        <p>
          このキーボードは<b>読めない漢字を1字入れるための道具</b>です。文章そのものは
          普段お使いのキーボードで打ち、読めない字に出くわしたときだけ切り替えてください。
        </p>
        <ol>
          <li>キーボードを「漢字カタチ入力」に切り替える</li>
          <li>かたちと部品で組む（読みから部品を出したいときは「読み」ボタン）</li>
          <li>候補をタップして<b>選ぶ</b> … この時点ではまだ文章に入りません</li>
          <li><b>送る</b>を押すと、カーソルの位置に入ります</li>
          <li><b>戻る</b>で元のキーボードへ。続けて何字も入れたいときは押さなくて大丈夫です</li>
        </ol>
        <p>
          組みかけのまま他のキーボードへ移っても、10分以内に戻ってくれば続きから打てます。
          押し間違いでいきなり文章に字が入ることはありません。
        </p>
      </Section>

      <Section title="システムキーボードとして使う">
        <ul>
          <li>
            <b>iOS</b>: 設定 → 一般 → キーボード → キーボード → 新しいキーボードを追加
            →「漢字カタチ入力」。「フルアクセスを許可」をオンにすると、アプリで調べた字の
            履歴・お気に入りがキーボードからも使えます（オフのままでも入力はできます。
            どちらでも通信は行いません）
          </li>
          <li>
            <b>Android</b>: 設定 → 言語と入力 → 画面キーボード → キーボードを管理
            →「漢字カタチ入力」をオン
          </li>
        </ul>
      </Section>

      <Section title="よくある質問">
        <p><b>候補が □ で表示されます</b></p>
        <p>
          拡張漢字（10万字のうち約9万字）は、端末に対応フォントが無いと □ になります。
          データとしては正しく入っているので、コピーして貼り付ければ正しい字が入ります。
        </p>
        <p><b>入力した内容はどこかに送られますか</b></p>
        <p>
          送られません。通信機能自体を実装していません。詳しくは
          <a href="/privacy">プライバシーポリシー</a>をご覧ください。
        </p>
        <p><b>探している漢字が出てきません</b></p>
        <p>
          部品を減らすか、分からない部分を <b>?</b>（なんでも）に置き換えてみてください。
          部品の形が辞書と少し違う場合もあるので、より単純な部品で試すのも有効です。
        </p>
      </Section>

      <Section title="お問い合わせ">
        <p>
          ご意見・不具合のご報告は <a href="mailto:upsee.haruki@gmail.com">upsee.haruki@gmail.com</a> までお願いします。
          返信までに数日いただくことがあります。
        </p>
      </Section>
    </StorePage>
  );
}
