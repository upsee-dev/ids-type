...
**文章は端末の普段のキーボードで打ち、読めない字のときだけこちらへ切り替える**道具に絞る。
かな漢字変換（連文節・単語変換）は作らない。同梱辞書は KANJIDIC2 系の1字ごとの読みしか
持たず、単語辞書を積むとライセンス・容量・変換品質の3つを同時に抱えることになるため。

流れは **組む → 選ぶ → 送る → 戻る**。相手のテキスト欄に触るのは「送る」の1か所だけで、
候補のタップは送る欄に入れるだけに留める（押し間違いを相手の本文に残さない）。
切り替えて戻る前提の道具なので、組みかけは打鍵のたびに保存し10分だけ復元できるようにする。
...
- 照合エンジンは TypeScript（`core/handwriting.ts`）・Kotlin（`Handwriting.kt`）・
  ...
  `native/ime/test/handwriting-cases.tsv` に書き出し、両言語のテストが解く）。
  浮動小数点の扱いを揃える3つの決まりは README とソース頭のコメントに書いた
- パターンは**手書きの面を初めて開いたときにだけ読む**。読み込み Android 53ms /
  iOS 21ms、照合 5ms / 2ms。iOS の60MB制限に収まるよう全字を1本のバイト列で持つ
  （約3.5MB）
- アプリの「読み」タブは OS キーボード頼みの TextInput をやめ、システムキーボードと
  同じ**12キーフリックを内蔵**した（表は `core/data/kana.ts`。Kotlin/Swift の Kana は
  引き続き手書きの二重管理なので、直すときは3か所）
- アプリは `react-native-svg` を追加したので**次の提出はネイティブビルドが必要**
  （OTAでは配れない）
