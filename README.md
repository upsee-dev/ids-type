...
- **かたち**（常時表示の1行）… 位置関係17種。「左右」「上下」「全かこみ」など。
  ...
  **タブに入れていない**のが要点で、「かたち→部品→部品」と続けて打つのに
  タブを往復させないため
- **よく使う部品**タブ … 辞書内で構成要素として登場する回数が多い順に180件
...
- 照合は `core/handwriting.ts`。字全体の外接枠で正規化（書く大きさ・位置に
  依存しない）→ 描いた画と参照の画を貪欲に対応づけて点距離を合計。**書き順違い・
  逆向きの画・続け書き**（口を2画で書く等。参照側の隣り合う2画をつなげた形とも
  照合する）を小さな加点つきで許す。画数の足切りと重心だけの粗選別を挟むので、
  1回の照合は数ms〜十数ms
- 手ぶれ・伸縮・逆順・書き順の入れ替えを加えた自己照合で top1 99%、続け書きでも
  top5 98%（`npm run test:handwriting` がしきい値で見張り、下回るとビルドを止める）
...
この読み込みとメモリを払わせないため。読み込みは Android 53ms・iOS 21ms、
1回の照合は Android 5ms・iOS 2ms。iOS は拡張の60MB制限があるので、
...
cd web && npm run test:handwriting          # 手書き照合の精度テスト(しきい値つき)
