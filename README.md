...
- **読みでさがす**タブ … パレットに無い部品を読みから引く

かたちコードの欄には**端末のIMEを触らせない**。触らせると変換が始まった瞬間に
欄ごと持っていかれ、先に選んだ〈左右〉が消える（React の制御された入力欄と
IMEの変換は相性が悪い）。読みから引きたいときは「読みでさがす」の独立した欄を使う。

アプリでは**入力欄にカーソルがあるあいだだけ**キーボードを出す（起動時は自動で出る）。
閉じているあいだは候補が広く見え、細い帯をタップすれば戻る。
「読みでさがす」でOSキーボードが出たときは、欄が隠れないよう画面を縮める
（Webは `interactive-widget` と visualViewport 追従、アプリは `KeyboardAvoidingView`）。
アプリで⌨（端末のキーボードで直接打つ）に入っているあいだも**かたちの行だけは畳まない**。
文字は端末のキーボード、かたちはタップ、と同時に打てないと〈左右〉を足すたびに往復になる。

### システムキーボードでの打ち方

アプリは「読めない字を調べる」道具だが、システムキーボードは**そのまま入力する**道具なので、
調べる→打つが1つの面で完結する必要がある。
...
- **組み立て途中のかたちコードは相手のテキスト欄に出さない**。キーボードの中だけで持ち、
  候補を選んで漢字1文字になった時点でカーソルへ転記する。以前 Android は composing text
  として相手の欄に下線つきで出していたが、検索欄などでは打っている端からオートコンプリートに
  巻き込まれた（iOS はもともとキーボード内で完結していた）
- **読みでさがす**タブは12キーフリックのかな面を内蔵する。自分が入力方式である以上、
  読みを打つために他のかなキーボードへ移ることはできない（移った時点で組み立て中の
  コードを持って行けない）。フリックの向きは指を離した時点で決まり、途中経過は読みの欄に
  色を変えて出す（ポップアップは端末によって位置がずれるので使わない）。
  フリックしない人向けに、同じキーの叩き直しで あ→い→う→え→お と送るトグルも受ける
- 引けた字は**タップでかたちコードに部品として足す**／**長押しでそのまま入力**
